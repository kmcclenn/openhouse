...
for (x <- 0 until 100) { spark.sql(s"INSERT INTO TABLE openhouse.testdb.tbA VALUES (current_timestamp(), 'testval1 $x', 'testval2 $x')") }
...
for (x <- 0 until 100) { spark.sql(s"INSERT INTO TABLE openhouse.testdb.tbB VALUES (current_timestamp(), $x)") }
